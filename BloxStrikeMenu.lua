...
local l_bar = Instance.new("Frame", l_main); l_bar.Size = UDim2.new(0,0,0,6); l_bar.Position = UDim2.new(0.1,0,0.7,0); l_bar.BackgroundColor3 = Color3.new(0,1,1); Instance.new("UICorner", l_bar)
local l_txt = Instance.new("TextLabel", l_main); l_txt.Size = UDim2.new(1,0,0.5,0); l_txt.Text = "INJECTING..."; l_txt.TextColor3 = Color3.new(0,1,1); l_txt.BackgroundTransparency = 1; l_txt.Font = Enum.Font.Code; l_txt.TextSize = 16

for i = 1, 100 do 
    l_bar.Size = UDim2.new(i/125, 0, 0, 6)
    l_txt.Text = "SYSTEM LOADING ["..i.."%]"
    task.wait(0.01) 
end
loader:Destroy()
...
local function addTgl(txt, var)
    local b = Instance.new("TextButton", scroll); b.Size = UDim2.new(1, 0, 0, 35); b.BackgroundColor3 = Color3.fromRGB(20, 20, 25); b.Text = txt; b.TextColor3 = Color3.new(1, 1, 1); b.Font = Enum.Font.Code; Instance.new("UICorner", b)
    b.MouseButton1Click:Connect(function()
        _G[var] = not _G[var]
        b.BackgroundColor3 = _G[var] and Color3.fromRGB(0, 120, 120) or Color3.fromRGB(20, 20, 25)
        Notify(txt, _G[var] and "ON" or "OFF")
    end)
end

addTgl("TRIGGER BOT", "TriggerBot")
addTgl("AIMBOT (RMB)", "Aimbot")
addTgl("FULL BRIGHT", "FullBright")
addTgl("FAST BHOP", "Bhop")
addTgl("OUTLINE WH", "OutlineEsp")

--------------------------------------------------
-- [ 5. ЦИКЛ ФУНКЦИЙ ]
--------------------------------------------------
rs.RenderStepped:Connect(function()
    if _G.FullBright then lighting.Brightness = 2; lighting.GlobalShadows = false end
    if _G.Bhop and uis:IsKeyDown(Enum.KeyCode.Space) then
        ...
        if h and hrp and h.FloorMaterial ~= Enum.Material.Air then h.Jump = true hrp.Velocity = hrp.Velocity + (h.MoveDirection * 1.7) end
    end
    if _G.Aimbot and uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
        ...
        if target then cam.CFrame = cam.CFrame:Lerp(CFrame.new(cam.CFrame.Position, target.Character.Head.Position), 0.15) end
    end

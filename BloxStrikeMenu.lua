--[[
    TERMINATOR v16.0 // ULTIMATE SURVIVOR
    - ALL FUNCTIONS INTEGRATED
    - INTERFACE: Loader (0-100%) + Notifications
    - COMBAT: TriggerBot, Aimbot, No-Shake, HitSounds
    - MOVEMENT: Velocity Bhop
...
local l_txt = Instance.new("TextLabel", l_main); l_txt.Size = UDim2.new(1,0,0.6,0); l_txt.Text = "STABILIZING GHOST..."; l_txt.TextColor3 = Color3.new(0,1,1); l_txt.BackgroundTransparency = 1; l_txt.Font = Enum.Font.Code; l_txt.TextSize = 17

for i = 1, 100 do 
    l_bar.Size = UDim2.new(i/125, 0, 0, 6)
    l_txt.Text = "LOADING v16.0 Ultimate ["..i.."%]"
    task.wait(0.015) 
end
loader:Destroy()
Notify("SYSTEM ONLINE", "Survivor Protocol: ACTIVE", Color3.new(0, 1, 0))
...
local sg = Instance.new("ScreenGui", cg); sg.Name = "Terminator_V16"; sg.ResetOnSpawn = false
...
rs.RenderStepped:Connect(function()
    -- TRIGGER BOT (Сам стреляет, когда враг в прицеле)
    if _G.TriggerBot then
        ...
        if target and target.Parent and target.Parent:FindFirstChild("Humanoid") then
            ...
            if hit_p and hit_p.Team ~= lp.Team then
                mouse1click() -- Команда выстрела
                if _G.HitSounds then PlayLocalSound("5143325717", 0.4) end -- Звук попадания (локальный)
            end
        end
    end

    -- BHOP (VELOCITY - Исправленный)
    ...
    -- NO CAMERA SHAKE (Убирает тряску)
    if _G.NoShake then
...
print("TERMINATOR v16.0 Ultimate Survivor Loaded.")
